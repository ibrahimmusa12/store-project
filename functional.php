...
function available_product_quantity($connect, $product_id)
{
	...
	inventory_order.inventory_order_status = '1'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
...
{
	$query = "
	SELECT * FROM user_details WHERE user_status='1'";
	$statement = $connect->prepare($query);
	$statement->execute();
...
{
	$query = "
	SELECT * FROM brand WHERE brand_status='1'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	return $statement->rowCount();
...
{
	$query = "
	SELECT * FROM tbl_products WHERE product_status='1'
	";
	$statement = $connect->prepare($query);
	$statement->execute();
	return $statement->rowCount();
...
function count_total_order_value($connect)
{
	...
	WHERE inventory_order_status='1'
	";
...
function count_total_cash_order_value($connect)
{
	$query = "
	SELECT sum(inventory_order_total) as total_order_value FROM inventory_order 
	WHERE payment_status = 'PayPal' 
	AND inventory_order_status='1'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= ' AND user_id = "'.$_SESSION["user_id"].'"';
...
{
	$query = "
	SELECT sum(inventory_order_total) as total_order_value FROM inventory_order WHERE payment_status = 'Card' AND inventory_order_status='1'
	";
	if($_SESSION['type'] == 'user')
	{
		$query .= ' AND user_id = "'.$_SESSION["user_id"].'"';
...
function get_user_wise_total_order($connect)
{
	...
	SUM(CASE WHEN inventory_order.payment_status = "PayPal" THEN inventory_order.inventory_order_total ELSE 0 END) AS cash_order_total, 
	SUM(CASE WHEN inventory_order.payment_status = "Card" THEN inventory_order.inventory_order_total ELSE 0 END) AS credit_order_total, 
	user_details.user_name 
	FROM inventory_order 
	INNER JOIN user_details ON user_details.user_id = inventory_order.user_id 
	WHERE inventory_order.inventory_order_status = "1" GROUP BY inventory_order.user_id
	';
	$statement = $connect->prepare($query);
	$statement->execute();
	$result = $statement->fetchAll();
	...
	<div class="table-responsive">
		<table class="table table-bordered table-striped">
			<tr>
				...
				<th>Total PayPal Order</th>
				<th>Total Card Order</th>
			</tr>
	';
